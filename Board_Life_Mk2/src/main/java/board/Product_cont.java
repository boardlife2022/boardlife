...
//데이터 연결, 전체 출력 메서드
	public ArrayList<Product> as() {
		...
		try {
			...
			conn = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/boardgame","root","rhkddml");
			if(conn == null) //conn 에 아무값이 없다면,
				throw new Exception("데이터베이스에 연결할 수 없습니다."); //이렇게 예외처리가 된다.
			stmt = (Statement) conn.createStatement();
			ResultSet rs = stmt.executeQuery("select*from product order by product_num desc;");
			while(rs.next())
			{	
	...
	public void in(Product pr) throws Exception {
		...
		try{
			...
			stmt=(Statement) conn.createStatement();
			String input = String.format("insert into product values ('%s','%s','%s','%s',%d,'%s',now(),'시바','%s','%s','%s','%s','%s','%s','%s','%s','%s')",product_num,product_pub,product_name,product_summary,product_price,product_lang,product_date,p_designer,p_year,p_theme,p_proceed,product_img,p_num,p_age,p_runtime,p_level,product_detail);
